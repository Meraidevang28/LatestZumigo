...
const ServiceSelection = ({navigation}) => {
  ...
  const fetchServiceData = async () => {
    try {
      ...
      const consultationUuid = 'a069b6c8-4ee2-41f6-ba3e-30b80d27';
      ...
      console.log('ServiceGroup UUID:', serviceGroupUuid);
  ...
  const toggleSelection = item => {
    console.log(item);
    setSelectedSpecialistService(item); // Set selected service

    // Replace the vaccines list with only the selected one
    setSelectedVaccines([item]);
  };
