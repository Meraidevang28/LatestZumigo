...
const VerifyOTP = ({navigation, route}) => {
  ...
  //     const deviceIp = await AsyncStorage.getItem('device_Ip');
  //     const authToken = await AsyncStorage.getItem('auth_token');
  //     console.log('Resending OTP with values:');
  //     console.log('Mobile:', mobile);
  //     console.log('UserTypeUuid:', userTypeUuid);
  ...
  const handleResend = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}${AUTH_SENDOTP}?${queryParams}`,
        {
          method: 'GET',
          headers: {
    ...
    if (enteredOtp === currentOtp) {
      if (isPartner) {
        try {
          ...
          const hasServices =
            (general && JSON.parse(general).length > 0) ||
          ...
          if (hasServices) {
            ...
            navigation.navigate(screens.Vetdetails); // fallback if no services selected
          }
        } catch (error) {
          console.error('Error checking stored services:', error);
          navigation.navigate('AddServices'); // fallback in case of error
        }
      } else {
        dispatch(login(user_type.PET_PARENT));
        navigation.navigate(screens.Dashboard);
